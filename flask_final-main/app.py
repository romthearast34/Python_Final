...
def submit_new_product():
    ...
    if 'cropped_image' in request.files:
        ...
        cropped_image.save(os.path.join(app.config['UPLOAD_FOLDER'], 'thumbnail', cropped_image_name))
    else:
...
def update():
    ...
    if cropped_image:
        ...
        cropped_image_path = os.path.join(app.config['UPLOAD_FOLDER'], 'thumbnail', cropped_image_filename)
